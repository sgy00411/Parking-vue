# 前后端热更新部署指南

## 📋 系统架构

```
用户访问: https://test001.cn (HTTPS 443)
           ↓
        Nginx 反向代理
           ↓
    ┌──────────┴──────────┐
    ↓                     ↓
前端软链接切换         后端Upstream负载均衡
    ↓                     ↓
dist-8081 ⇄ dist-8082  8086 ⇄ 8087
```

---

## 🚀 一、前端热更新

### 目录结构
```
/www/wwwroot/Parking-vue/
├── dist -> dist-8081      # 软链接指向当前版本
├── dist-8081/             # 版本1（当前）
├── dist-8082/             # 版本2（备用）
├── src/                   # 源代码
└── deploy-frontend.sh     # 热更新脚本
```

### 更新步骤

#### 方式1：使用一键脚本（推荐）

```bash
cd /www/wwwroot/Parking-vue
./deploy-frontend.sh
```

脚本会自动：
1. 检测当前版本（8081/8082）
2. 构建新版本 `npm run build`
3. 部署到备用目录
4. 切换软链接
5. 重载nginx

#### 方式2：手动部署

```bash
cd /www/wwwroot/Parking-vue

# 1. 拉取代码
git pull

# 2. 安装依赖（如果package.json有变化）
npm install

# 3. 构建
npm run build

# 4. 检查当前版本
ls -l dist  # 查看软链接指向

# 5. 部署到备用目录
# 如果当前是dist-8081，则部署到dist-8082
rm -rf dist-8082/*
cp -r dist/* dist-8082/

# 6. 切换软链接（原子操作）
ln -snf dist-8082 dist

# 7. 重载nginx
nginx -t && nginx -s reload
```

### 回滚操作

```bash
cd /www/wwwroot/Parking-vue

# 切换回旧版本
ln -snf dist-8081 dist

# 重载nginx
nginx -s reload
```

---

## 🔧 二、后端热更新

### 配置文件
```
/www/wwwroot/Parking-java/
├── src/main/resources/
│   ├── application.yml        # 8086配置
│   └── application-8087.yml   # 8087配置
├── target/
│   └── quaer_api-0.0.1-SNAPSHOT.jar
└── deploy-backend.sh          # 热更新脚本
```

### 关键配置差异

**application.yml (8086)**
```yaml
server:
  port: 8086

mqtt:
  client-id: parking_server
```

**application-8087.yml (8087)**
```yaml
server:
  port: 8087

mqtt:
  client-id: parking_server_8087  # 必须不同，避免MQTT冲突
```

### 更新步骤

#### 方式1：使用一键脚本（推荐）

```bash
cd /www/wwwroot/Parking-java
./deploy-backend.sh
```

脚本会自动：
1. 检测当前主服务端口
2. 构建新版本 `mvn clean package`
3. 停止备用端口旧服务
4. 启动新服务
5. 健康检查
6. 切换nginx流量
7. 停止旧服务

#### 方式2：手动部署

```bash
cd /www/wwwroot/Parking-java

# 1. 拉取代码
git pull

# 2. 构建
mvn clean package -DskipTests

# 3. 检查当前主服务
netstat -tlnp | grep java
# 假设8086是主服务

# 4. 启动8087
nohup java -jar target/quaer_api-0.0.1-SNAPSHOT.jar \
  --spring.config.location=classpath:/application-8087.yml \
  > logs/app-8087.log 2>&1 &

# 5. 等待启动并检查
sleep 10
netstat -tlnp | grep 8087
tail -f logs/app-8087.log

# 6. 修改nginx配置
vim /www/server/panel/vhost/nginx/122.51.64.122.conf

# 修改upstream配置：
# server 127.0.0.1:8086 down max_fails=3 fail_timeout=30s;
# server 127.0.0.1:8087 weight=1 max_fails=3 fail_timeout=30s;

# 7. 重载nginx
nginx -t && nginx -s reload

# 8. 等待旧服务处理完请求
sleep 10

# 9. 停止8086
kill -15 $(lsof -ti:8086)
```

### 回滚操作

```bash
# 1. 重新启动旧端口
cd /www/wwwroot/Parking-java
nohup java -jar target/quaer_api-0.0.1-SNAPSHOT.jar \
  > logs/app-8086.log 2>&1 &

# 2. 修改nginx配置切换回8086
vim /www/server/panel/vhost/nginx/122.51.64.122.conf
# 改为：
# server 127.0.0.1:8086 weight=1 max_fails=3 fail_timeout=30s;
# server 127.0.0.1:8087 down max_fails=3 fail_timeout=30s;

# 3. 重载nginx
nginx -s reload

# 4. 停止8087
kill -15 $(lsof -ti:8087)
```

---

## 📊 三、状态检查

### 检查前端版本
```bash
ls -l /www/wwwroot/Parking-vue/dist
# 输出: dist -> dist-8081  或  dist -> dist-8082
```

### 检查后端服务
```bash
# 查看Java进程
netstat -tlnp | grep java

# 查看nginx配置
grep -A 10 "upstream backend_app" /www/server/panel/vhost/nginx/122.51.64.122.conf

# 查看日志
tail -f /www/wwwroot/Parking-java/logs/app-8086.log
tail -f /www/wwwroot/Parking-java/logs/app-8087.log
```

### 测试服务
```bash
# 测试前端
curl -I https://test001.cn

# 测试后端API
curl https://test001.cn/api/health  # 如果有健康检查接口
```

---

## ⚠️ 四、注意事项

### 前端注意事项
1. **缓存问题**：确保在构建时设置正确的版本号或hash，避免浏览器缓存
2. **静态资源**：软链接切换是原子操作，不会出现资源混乱
3. **构建失败**：脚本会自动检测，不会切换到错误版本

### 后端注意事项
1. **MQTT冲突**：两个实例的`mqtt.client-id`必须不同
2. **数据库连接**：共用同一个数据库，注意并发
3. **健康检查**：确保新服务完全启动后再切换流量
4. **会话保持**：nginx配置了`ip_hash`，同一客户端请求会到同一服务器
5. **端口占用**：部署前确保目标端口没有旧进程

### 通用注意事项
1. **备份**：脚本会自动备份nginx配置
2. **日志**：所有操作都有详细日志记录
3. **回滚**：保留旧版本，支持快速回滚
4. **测试**：先在测试环境验证，再生产部署

---

## 🔍 五、常见问题

### Q1: 前端更新后仍显示旧版本？
A: 浏览器缓存问题，清除缓存或强制刷新（Ctrl+F5）

### Q2: 后端切换失败？
A: 检查新服务是否正常启动：
```bash
netstat -tlnp | grep 8087
tail -f logs/app-8087.log
```

### Q3: nginx配置测试失败？
A: 脚本会自动回滚配置，检查语法错误：
```bash
nginx -t
```

### Q4: 两个后端实例能同时运行吗？
A: 可以，但MQTT会冲突。切换时短暂重叠没问题，长期运行需调整MQTT配置

### Q5: 如何查看部署历史？
A: 查看nginx配置备份：
```bash
ls -lh /www/server/panel/vhost/nginx/122.51.64.122.conf.backup-*
```

---

## 📞 六、紧急情况处理

### 服务完全宕机
```bash
# 1. 检查nginx
systemctl status nginx
nginx -t

# 2. 检查后端
netstat -tlnp | grep java

# 3. 重启服务
cd /www/wwwroot/Parking-java
nohup java -jar target/quaer_api-0.0.1-SNAPSHOT.jar > logs/app.log 2>&1 &

# 4. 重启nginx
nginx -s reload
```

### 回滚到最后的稳定版本
```bash
# 前端
cd /www/wwwroot/Parking-vue
ln -snf dist-8081 dist  # 或 dist-8082
nginx -s reload

# 后端
# 恢复nginx配置备份
cp /www/server/panel/vhost/nginx/122.51.64.122.conf.backup-XXXXXX \
   /www/server/panel/vhost/nginx/122.51.64.122.conf
nginx -s reload
```

---

## 📝 七、更新日志模板

建议每次部署后记录：

```
日期：2025-XX-XX
更新类型：前端/后端
版本：dist-8081/8082 或 8086/8087
更新内容：
- 功能1
- 功能2
- Bug修复

部署人员：
备注：
```

---

**最后更新：2025-12-28**
